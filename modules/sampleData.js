export { getSampleData }
const getSampleData = () => {
    return sample
}

const technologies = [
    {
        label: "Kubernetes", category: "infrastructure", image: "data:image/png;base64,"
        , homepage: "", tags: ["container", "cncf", "open source", "cluster"], offering:"oss"
    },

    { label: "Java", category: "language", image: "https://logoeps.com/wp-content/uploads/2013/03/java-eps-vector-logo.png", homepage: "", tags: ['programming', 'java', 'open source', 'oo'] , offering:"oss"},
    { label: "Python", category: "language", image: "", homepage: "", tags: ["", ""] , offering:"oss"},
    {
        label: "PostgreSQL", category: "database", image: "https://w7.pngwing.com/pngs/559/367/png-transparent-postgresql-object-relational-database-oracle-database-freebsd-icon-text-logo-head.png"
        , homepage: "https://www.postgresql.org/", tags: ["open source", "rdbms", "sql", "acid"], offering:"oss", description:`PostgreSQL also known as Postgres, is a free and open-source relational database management system (RDBMS) emphasizing extensibility and SQL compliance. It was originally named POSTGRES, referring to its origins as a successor to the Ingres database developed at the University of California, Berkeley. In 1996, the project was renamed to PostgreSQL to reflect its support for SQL. After a review in 2007, the development team decided to keep the name PostgreSQL and the alias Postgres.
        PostgreSQL features transactions with Atomicity, Consistency, Isolation, Durability (ACID) properties, automatically updatable views, materialized views, triggers, foreign keys, and stored procedures. It is designed to handle a range of workloads, from single machines to data warehouses or Web services with many concurrent users. It is the default database for macOS Server and is also available for Windows, Linux, FreeBSD, and OpenBSD.`
        , vendor: "PostgreSQL Global Development Group"
    },
    {
        label: "Elasticsearch", category: "database", image: "data:image/png;base64,"
        , homepage: "", tags: ['data', 'data analytics', 'nosql', 'database'], offering:"oss"
    },
    {
        label: "Redis", category: "database", image: "https://www.nditech.org/sites/default/files/styles/small_photo/public/redis-logo.png?itok=LrULOkWT"
        , homepage: "", tags: ['data', 'database', 'cache', 'open source'], offering:"oss"
    },
    {
        label: "Oracle Database", category: "database", image: "https://i1.wp.com/deepinthecode.com/wp-content/uploads/2018/09/Oracle_Database.png?fit=432%2C203&ssl=1"
        , homepage: "", tags: ['data', 'database', 'sql', 'rdbms', 'oracle'], offering:"commercial"
    },
    {
        label: "Node.js", category: "language", image: "data:image/png;base64,"
        , homepage: "", tags: ['programming', 'open source'], offering:"oss"
    }
    ,{label:"Snowflake", homepage:"https://www.snowflake.com/", offering:"commercial", tags:['data','cloud','saas','data warehouse','data share','data cloud']}
    ,{label:"Hashicorp Vault", offering:"oss", tags:['secret',"vault"]}
    ,{label:"AWS KMS", offering:"commercial", tags:['aws','key','secret','cloud']}
    ,{label:"AWS Secrets Management", offering:"commercial", tags:['aws','cloud','key']}
    ,{label:"GCP Secrets Management", offering:"commercial", tags:['gcp','cloud','key']}
    ,{label:"Azure Key Vault", offering:"commercial", tags:['azure','cloud','key','secret','certificate']}
    ,{label:"Cassandra", offering:"oss", tags:['database','sql','distributed','acid']}
    ,{label:"MySQL", offering:"oss", tags:['sql','database','data','acid']}
    ,{label:"Apache Kafka", offering:"oss", tags:['data','event','distributed','apache','queue','topic']}
    ,{label:"Memcached", offering:"oss", tags:['cache','data','memory','distributed']}
    ,{label:"Go", offering:"oss", tags:['golang','google']}
    ,{label:"Rust", offering:"oss", tags:['rust','programming']}
    ,{label:"Ruby", offering:"oss", tags:['ruby','programming','rails']}
    ,{label:"MariaDB", offering:"oss", tags:['mysql','fork','data','sql','acid']}
    ,{label:"Elastic", offering:"oss", tags:['monitoring','log','dashboard','logstash','kibana']}
    ,{label:"Grafana", offering:"oss", tags:['metric','monitoring','dashboard','alert']}
    ,{label:"Prometheus", offering:"oss", tags:['metric','monitoring','data','timeseries','database']}
    ,{label:"Datadog", offering:"commercial", tags:['monitoring','metric','log']}
    ,{label:"OpenMetrics", offering:"oss", tags:['metric','monitoring','standard']}
    ,{label:"Splunk", offering:"commercial", tags:['metric','monitoring','data','timeseries','database']}
    ,{label:"Cloudwatch", offering:"commercial", tags:['aws','metric','monitoring','data']}
    ,{label:"Jaeger", offering:"oss", tags:['metric','monitoring','data','cncf','kubernetes']}
    ,{label:"Kiali", offering:"oss", tags:['metric','monitoring','data','dashboard','servicemesh','observability']}
    ,{label:"Thanos", offering:"oss", tags:['metric','monitoring','data','prometheus','ha']}
    ,{label:"Flux", offering:"oss", tags:['cd','gitops','kubernetes','fluxcd']}
    ,{label:"Circle CI", offering:"oss", tags:['cd','ci']}
    ,{label:"GitLab", offering:"oss", tags:['git','cd','ci','source code control']}
    ,{label:"Kustomize", offering:"oss", tags:['git','cd','ci']}
    ,{label:"Helm", offering:"oss", tags:['kubernetes','package manager','charts','cncf']}
    ,{label:"Jenkins", offering:"oss", tags:['ci','cd','build','automation']}
    ,{label:"Spinnaker", offering:"oss", tags:['ci','cd','build','automation']}
    ,{label:"Team City", offering:"oss", tags:['ci','cd','build','automation']}
    ,{label:"Azure DevOps", offering:"commercial", tags:['azure','arm','terraform','provision','ci','cd','build','automation']}
    ,{label:"GitHub Actions", offering:"oss", tags:['ci','cd','build','automation']}
    ,{label:"Tekton CD", offering:"oss", tags:['ci','cd','build','automation']}
    ,{label:"Travis CI", offering:"oss", tags:['ci','cd','build','automation']}
    
]

// these genericRatingProperties are applied to every ratingType
const genericRatingProperties = [{ name: "scope", description: "The scope or context to which the rating applies" }
    , { name: "author", description: "The name of the person who made the judgement" }
    , { name: "timestamp", description: "When was this rating defined" }
    , { name: "tags", description: "Which tags are associated with this rating (tags are for example used for thematic filtering of ratings)" }
    , { name: "comment", description: "Additional remark regarding this rating" }
]

const model =
{
    objectTypes: [
        { name: "technology", properties: [] },
        { name: "consultant" },
        { name: "workitem" }
    ]
    , ratingType: [
        {
            name: "technologyAdoption", objectType: "technology", properties: [
                {
                    name: "ambition", description: "The current outlook or intent regarding this technology", defaultValue: "identified"
                    , values: [{ value: "identified", label: "Identified" }, { value: "hold", label: "Hold" }, { value: "assess", label: "Assess" }, { value: "adopt", label: "Adopt" }]
                },
                {
                    name: "magnitude", description: "The relative size of the technology (in terms of investment, people involved, percentage of revenue)", defaultValue: "medium"
                    , values: [{ value: "tiny", label: "Tiny or Niche" }, { value: "medium", label: "Medium" }, { value: "large", label: "Large" }]
                }

            ]
        }
        , { name: "cvRating", objectType: "technology", properties: [] }
        , { name: "allocationPipeline", objectType: "consultant", properties: [] }
        , { name: "progressStatus", objectType: "workitem", properties: [] }
    ]
}

// generate a viewpoint: select radar template, select rating type (and indirect object type), define filter - to restrict objects & ratings)
//                       select viewpoint template - which defines mapping of properties to visual characteristics (sector, ring, shape, color, size, ..)
//                       define viewpoint properties: title, description, visual overrides

const viewpoints = [
    {
        name: "My Technology Radar - Integration"
        , template: null
        , ratingTypes: []  // which rating type(s) - for which objectTypes - are displayed
        , propertyVisualMaps: { // mapping between property values in rating and object on the one hand and the corresponding visual elements on the other sectors, rings, shapes, colors, sizes ;
                                // which property value maps to which of visual elements (indicated by their sequence number in th template) 
            // note: the order of elements in these maps drives the order in which color/size/shape elements are shown in legend and context menu
            sizeMap: { "tiny": 0, "medium": 1, "large": 2 } // the rating magnitude property drives the size; the values of magnitude are mapped to values for size
            , sectorMap: { "database": 0, "language": 3, "infrastructure": 2,"concepts": 4,"libraries":1 } // the object category property drives the sector; the values of category are mapped to values for sector
            , ringMap: { "hold": 0, "assess": 1, "adopt": 3,"trial":2 } // the rating ambition property drives the ring; the values of ambition are mapped to values for ring
            , shapeMap: {"oss" : 1, "commercial" : 0 ,"other":3}
            , colorMap: { "short": 0, "long":1,"intermediate":3, "other":2}
        },
        blipDisplaySettings: {showImages: false, showShapes: true, showLabels:true
            , applyShapes:true, applySizes:true, applyColors:true, tagFilter:""}
        //for example: property category in objectType technology is mapped to sector in radar
        // the specific value mapping: maps technology.category values to sectors in the selected radar template
        // one of the sectors can be used to assign "others" - any value not already explicity mapped
        // when there is no "others" sector indicated or a technology.category is explicitly not mapped, then the corresponding blips are not visible
        , blips: [ // derived 
            { id: "1", rating: null, x: 300, y: 200, hidden: false },
        ]
    }
]

const generateBlips = () => {
    // for all technologies
    // create a rating
    // and for each rating, create a blip
    const blips = []
    for (let i = 0; i < technologies.length; i++) {
        const object = technologies[i]
        const rating = {
           // ambition: Math.random() < 0.3 ? "spotted" : (Math.random() < 0.5 ? "assess" : "adopt")
             magnitude: Math.random() < 0.01 ? "medium" : (Math.random() < 0.9 ? "tiny" : "large")
            , experience: Math.random() < 0.01 ? "short" : (Math.random() < 0.9 ? "long" : "medium")
            , timestamp : Date.now()
            , scope: "Conclusion"
            , comment : "no comment yet"
            , author : "system generated"
            , object: object
        }
        if (object.category==null) {object.category="infrastructure"}
        const blip = { id: `${i}`, rating: rating, }
        blips.push(blip)
    }
    return blips
}

viewpoints[0].blips = generateBlips()

/*
rating : object - ratingType: property assignments (timestamp, scope, scorer, tags, notes)

viewpoint (actual representation on template of ratings)
- title/description
- based on radar template
- contains blips - visual mapping of ratings (x,y, color/shape/size, label )  

snapshot = viewpoint frozen in time

*/


const sample = {
    "viewpoints": [
        {
            "name": "My Technology Radar - Integration",
            "template": {
                "svg_id": "radarSVGContainer",
                "width": 1450,
                "height": 1100,
                "topLayer": "sectors",
                "selectedRing": 1,
                "selectedSector": 2,
                "rotation": 0,
                "maxRingRadius": 450,
                "sectorBoundariesExtended": true,
                "editMode": false,
                "defaultFont": {
                    "color": "blue",
                    "fontSize": "38px",
                    "fontFamily": "Arial, Helvetica",
                    "fontStyle": "italic",
                    "fontWeight": "bold"
                },
                "title": {
                    "text": "Conclusion Technology Radar",
                    "x": -700,
                    "y": -520,
                    "font": {
                        "fontSize": "34px",
                        "fontFamily": "Courier"
                    }
                },
                "colors": {
                    "background": "#fFf",
                    "grid": "#bbb",
                    "inactive": "#ddd"
                },
                "ringConfiguration": {
                    "outsideRingsAllowed": true,
                    "font": {
                        "color": "purple",
                        "fontSize": "24px",
                        "fontFamily": "Arial, Helvetica",
                        "fontStyle": "normal",
                        "fontWeight": "normal"
                    },
                    "stroke": {
                        "strokeWidth": 4,
                        "strokeColor": "blue",
                        "strokeArray": "100 1"
                    },
                    "rings": [
                        {
                            "label": "Hold",
                            "width": 0.2
                        },
                        {
                            "label": "Assess",
                            "width": 0.2
                        },
                        {
                            "label": "Trial",
                            "width": 0.15
                        },
                        {
                            "label": "Adopt",
                            "width": 0.45
                        }
                    ]
                },
                "sectorConfiguration": {
                    "outsideSectorsAllowed": true,
                    "font": {
                        "color": "#000",
                        "fontSize": "28px",
                        "fontFamily": "Arial, Helvetica",
                        "fontStyle": "normal",
                        "fontWeight": "normal"
                    },
                    "stroke": {
                        "strokeWidth": 2,
                        "strokeColor": "gray",
                        "strokeArray": "100 1"
                    },
                    "sectors": [
                        {
                            "label": "Data Management",
                            "angle": 0.2,
                            "backgroundImage": {}
                        },
                        {
                            "label": "Libraries & Frameworks",
                            "angle": 0.2,
                            "backgroundImage": {}
                        },
                        {
                            "label": "Infrastructure",
                            "angle": 0.25
                        },
                        {
                            "label": "Languages",
                            "angle": 0.2
                        },
                        {
                            "label": "Concepts & Methodology",
                            "angle": 0.15
                        }
                    ]
                },
                "colorsConfiguration": {
                    "label": "Maturity",
                    "colors": [
                        {
                            "label": "Fresh",
                            "color": "green",
                            "enabled": true
                        },
                        {
                            "label": "Been Around",
                            "color": "blue",
                            "enabled": true
                        },
                        {
                            "label": "Very Mature",
                            "color": "gray",
                            "enabled": true
                        },
                        {
                            "label": "Intermediate",
                            "color": "pink",
                            "enabled": true
                        },
                        {
                            "label": "Unassigned",
                            "color": "white"
                        }
                    ]
                },
                "sizesConfiguration": {
                    "label": "Relevance",
                    "sizes": [
                        {
                            "label": "Niche",
                            "size": 0.7,
                            "enabled": true
                        },
                        {
                            "label": "Medium    ",
                            "size": 0.9,
                            "enabled": false
                        },
                        {
                            "label": "Very relevant",
                            "size": 1.3,
                            "enabled": true
                        },
                        {
                            "label": "Regular",
                            "size": 4
                        },
                        {
                            "label": "Regular",
                            "size": 5,
                            "enabled": false
                        }
                    ]
                },
                "shapesConfiguration": {
                    "label": "Offering",
                    "shapes": [
                        {
                            "label": "Commercial",
                            "shape": "square"
                        },
                        {
                            "label": "Open Source",
                            "shape": "diamond"
                        },
                        {
                            "label": "Label",
                            "shape": "rectangleHorizontal",
                            "enabled": false
                        },
                        {
                            "label": "Other",
                            "shape": "circle",
                            "enabled": false
                        },
                        {
                            "label": "Label",
                            "shape": "star",
                            "enabled": false
                        },
                        {
                            "label": "Label",
                            "shape": "rectangleVertical",
                            "enabled": false
                        },
                        {
                            "label": "Label",
                            "shape": "triangle",
                            "enabled": false
                        },
                        {
                            "label": "Label",
                            "shape": "ring",
                            "enabled": false
                        },
                        {
                            "label": "Label",
                            "shape": "plus",
                            "enabled": false
                        }
                    ]
                }
            },
            "ratingTypes": [],
            "propertyVisualMaps": {
                "sizeMap": {
                    "tiny": 0,
                    "medium": 1,
                    "large": 2
                },
                "sectorMap": {
                    "database": 0,
                    "language": 3,
                    "infrastructure": 2,
                    "concepts": 4,
                    "libraries": 1
                },
                "ringMap": {
                    "hold": 0,
                    "assess": 1,
                    "adopt": 3,
                    "trial": 2
                },
                "shapeMap": {
                    "oss": 1,
                    "commercial": 0,
                    "other": 3
                },
                "colorMap": {
                    "short": 0,
                    "long": 1,
                    "intermediate": 3,
                    "other": 2
                }
            },
            "blipDisplaySettings": {
                "showImages": false,
                "showShapes": true,
                "showLabels": true,
                "applyShapes": true,
                "applySizes": true,
                "applyColors": true,
                "tagFilter": ""
            },
            "blips": [
                {
                    "id": "0",
                    "rating": {
                        "magnitude": "medium",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Kubernetes",
                            "category": "infrastructure",
                            "image": "data:image/png;base64,",
                            "homepage": "",
                            "tags": [
                                "container",
                                "cncf",
                                "open source",
                                "cluster"
                            ],
                            "offering": "oss"
                        },
                        "ambition": "adopt"
                    },
                    "x": -172.37005615234375,
                    "y": 13.581069946289062
                },
                {
                    "id": "1",
                    "rating": {
                        "magnitude": "large",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Java",
                            "category": "language",
                            "image": "https://logoeps.com/wp-content/uploads/2013/03/java-eps-vector-logo.png",
                            "homepage": "",
                            "tags": [
                                "programming",
                                "java",
                                "open source",
                                "oo"
                            ],
                            "offering": "oss"
                        },
                        "ambition": "adopt"
                    },
                    "x": -0.075042724609375,
                    "y": 75.29833984375
                },
                {
                    "id": "2",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Python",
                            "category": "language",
                            "image": "",
                            "homepage": "",
                            "tags": [
                                "",
                                ""
                            ],
                            "offering": "oss"
                        },
                        "ambition": "adopt"
                    },
                    "x": 29.941268920898438,
                    "y": 168.69598388671875
                },
                {
                    "id": "3",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "PostgreSQL",
                            "category": "database",
                            "image": "https://w7.pngwing.com/pngs/559/367/png-transparent-postgresql-object-relational-database-oracle-database-freebsd-icon-text-logo-head.png",
                            "homepage": "https://www.postgresql.org/",
                            "tags": [
                                "open source",
                                "rdbms",
                                "sql",
                                "acid"
                            ],
                            "offering": "oss",
                            "description": "PostgreSQL also known as Postgres, is a free and open-source relational database management system (RDBMS) emphasizing extensibility and SQL compliance. It was originally named POSTGRES, referring to its origins as a successor to the Ingres database developed at the University of California, Berkeley. In 1996, the project was renamed to PostgreSQL to reflect its support for SQL. After a review in 2007, the development team decided to keep the name PostgreSQL and the alias Postgres.\n        PostgreSQL features transactions with Atomicity, Consistency, Isolation, Durability (ACID) properties, automatically updatable views, materialized views, triggers, foreign keys, and stored procedures. It is designed to handle a range of workloads, from single machines to data warehouses or Web services with many concurrent users. It is the default database for macOS Server and is also available for Windows, Linux, FreeBSD, and OpenBSD.",
                            "vendor": "PostgreSQL Global Development Group"
                        },
                        "ambition": "adopt"
                    },
                    "x": 66.08761596679688,
                    "y": -108.48751831054688
                },
                {
                    "id": "4",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Elasticsearch",
                            "category": "database",
                            "image": "data:image/png;base64,",
                            "homepage": "",
                            "tags": [
                                "data",
                                "data analytics",
                                "nosql",
                                "database"
                            ],
                            "offering": "oss"
                        },
                        "ambition": "adopt"
                    },
                    "x": 142.92044067382812,
                    "y": -92.12216186523438
                },
                {
                    "id": "5",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Redis",
                            "category": "database",
                            "image": "https://www.nditech.org/sites/default/files/styles/small_photo/public/redis-logo.png?itok=LrULOkWT",
                            "homepage": "",
                            "tags": [
                                "data",
                                "database",
                                "cache",
                                "open source"
                            ],
                            "offering": "oss"
                        },
                        "ambition": "adopt"
                    },
                    "x": 80.98797607421875,
                    "y": -150.53501892089844
                },
                {
                    "id": "6",
                    "rating": {
                        "magnitude": "large",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Oracle Database",
                            "category": "database",
                            "image": "https://i1.wp.com/deepinthecode.com/wp-content/uploads/2018/09/Oracle_Database.png?fit=432%2C203&ssl=1",
                            "homepage": "",
                            "tags": [
                                "data",
                                "database",
                                "sql",
                                "rdbms",
                                "oracle"
                            ],
                            "offering": "commercial"
                        },
                        "ambition": "adopt"
                    },
                    "x": 66.33810424804688,
                    "y": -31.947418212890625
                },
                {
                    "id": "7",
                    "rating": {
                        "magnitude": "large",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Node.js",
                            "category": "language",
                            "image": "data:image/png;base64,",
                            "homepage": "",
                            "tags": [
                                "programming",
                                "open source"
                            ],
                            "offering": "oss"
                        },
                        "ambition": "adopt"
                    },
                    "x": -23.671669006347656,
                    "y": 154.03582763671886
                },
                {
                    "id": "8",
                    "rating": {
                        "magnitude": "medium",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Snowflake",
                            "homepage": "https://www.snowflake.com/",
                            "offering": "commercial",
                            "tags": [
                                "data",
                                "cloud",
                                "saas",
                                "data warehouse",
                                "data share",
                                "data cloud"
                            ],
                            "category": "database"
                        },
                        "ambition": "assess"
                    },
                    "x": 294.3595275878906,
                    "y": -87.16007995605469
                },
                {
                    "id": "9",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Hashicorp Vault",
                            "offering": "oss",
                            "tags": [
                                "secret",
                                "vault"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "assess"
                    },
                    "x": -273.55972290039057,
                    "y": 178.113037109375
                },
                {
                    "id": "10",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "AWS KMS",
                            "offering": "commercial",
                            "tags": [
                                "aws",
                                "key",
                                "secret",
                                "cloud"
                            ],
                            "category": "infrastructure"
                        }
                    }
                },
                {
                    "id": "11",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "AWS Secrets Management",
                            "offering": "commercial",
                            "tags": [
                                "aws",
                                "cloud",
                                "key"
                            ],
                            "category": "infrastructure"
                        }
                    },
                    "x": -518.1266479492188,
                    "y": 177.45331573486328
                },
                {
                    "id": "12",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "medium",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "GCP Secrets Management",
                            "offering": "commercial",
                            "tags": [
                                "gcp",
                                "cloud",
                                "key"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "hold"
                    },
                    "x": -389.38272094726557,
                    "y": -34.110748291015625
                },
                {
                    "id": "13",
                    "rating": {
                        "magnitude": "medium",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Azure Key Vault",
                            "offering": "commercial",
                            "tags": [
                                "azure",
                                "cloud",
                                "key",
                                "secret",
                                "certificate"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "adopt"
                    },
                    "x": -127.17877197265625,
                    "y": 105.87071228027344
                },
                {
                    "id": "14",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Cassandra",
                            "offering": "oss",
                            "tags": [
                                "database",
                                "sql",
                                "distributed",
                                "acid"
                            ],
                            "category": "database"
                        },
                        "ambition": "assess"
                    },
                    "x": 249.75613403320312,
                    "y": -164.6012725830078
                },
                {
                    "id": "15",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "MySQL",
                            "offering": "oss",
                            "tags": [
                                "sql",
                                "database",
                                "data",
                                "acid"
                            ],
                            "category": "database"
                        },
                        "ambition": "adopt"
                    },
                    "x": 172.04534912109375,
                    "y": -24.980953216552734
                },
                {
                    "id": "16",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Apache Kafka",
                            "offering": "oss",
                            "tags": [
                                "data",
                                "event",
                                "distributed",
                                "apache",
                                "queue",
                                "topic"
                            ],
                            "category": "database"
                        },
                        "ambition": "adopt"
                    },
                    "x": 123.04705810546875,
                    "y": -42.232711791992244
                },
                {
                    "id": "17",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Memcached",
                            "offering": "oss",
                            "tags": [
                                "cache",
                                "data",
                                "memory",
                                "distributed"
                            ],
                            "category": "database"
                        },
                        "ambition": "trial"
                    },
                    "x": 232.90435791015625,
                    "y": -23.87445068359375
                },
                {
                    "id": "18",
                    "rating": {
                        "magnitude": "large",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Go",
                            "offering": "oss",
                            "tags": [
                                "golang",
                                "google"
                            ],
                            "category": "language"
                        },
                        "ambition": "assess"
                    },
                    "x": -45.76849365234375,
                    "y": 319.8363342285156
                },
                {
                    "id": "19",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Rust",
                            "offering": "oss",
                            "tags": [
                                "rust",
                                "programming"
                            ],
                            "category": "language"
                        },
                        "ambition": "hold"
                    },
                    "x": -202.85687255859375,
                    "y": 341.788330078125
                },
                {
                    "id": "20",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Ruby",
                            "offering": "oss",
                            "tags": [
                                "ruby",
                                "programming",
                                "rails"
                            ],
                            "category": "language"
                        },
                        "ambition": "hold"
                    },
                    "x": -129.833740234375,
                    "y": 373.9172134399414
                },
                {
                    "id": "21",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "MariaDB",
                            "offering": "oss",
                            "tags": [
                                "mysql",
                                "fork",
                                "data",
                                "sql",
                                "acid"
                            ],
                            "category": "database"
                        }
                    },
                    "x": 567.8643798828125,
                    "y": -22.160247802734375
                },
                {
                    "id": "22",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "medium",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Elastic",
                            "offering": "oss",
                            "tags": [
                                "monitoring",
                                "log",
                                "dashboard",
                                "logstash",
                                "kibana"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "adopt"
                    },
                    "x": -179.63629150390625,
                    "y": -43.380035400390625
                },
                {
                    "id": "23",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Grafana",
                            "offering": "oss",
                            "tags": [
                                "metric",
                                "monitoring",
                                "dashboard",
                                "alert"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "adopt"
                    },
                    "x": -88.72244262695312,
                    "y": 58.00176811218262
                },
                {
                    "id": "24",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "medium",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Prometheus",
                            "offering": "oss",
                            "tags": [
                                "metric",
                                "monitoring",
                                "data",
                                "timeseries",
                                "database"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "adopt"
                    },
                    "x": -158.366455078125,
                    "y": 71.62266540527344
                },
                {
                    "id": "25",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Datadog",
                            "offering": "commercial",
                            "tags": [
                                "monitoring",
                                "metric",
                                "log"
                            ],
                            "category": "infrastructure"
                        }
                    },
                    "x": -565.4864501953125,
                    "y": 272.88299560546875
                },
                {
                    "id": "26",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "OpenMetrics",
                            "offering": "oss",
                            "tags": [
                                "metric",
                                "monitoring",
                                "standard"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "assess"
                    },
                    "x": -293.05737304687494,
                    "y": -139.82057189941406
                },
                {
                    "id": "27",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "medium",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Splunk",
                            "offering": "commercial",
                            "tags": [
                                "metric",
                                "monitoring",
                                "data",
                                "timeseries",
                                "database"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "trial"
                    },
                    "x": -214.5661010742187,
                    "y": -106.29897785186768
                },
                {
                    "id": "28",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Cloudwatch",
                            "offering": "commercial",
                            "tags": [
                                "aws",
                                "metric",
                                "monitoring",
                                "data"
                            ],
                            "category": "infrastructure"
                        }
                    },
                    "x": -500.03704833984375,
                    "y": 360.0298767089844
                },
                {
                    "id": "29",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "medium",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Jaeger",
                            "offering": "oss",
                            "tags": [
                                "metric",
                                "monitoring",
                                "data",
                                "cncf",
                                "kubernetes"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "assess"
                    },
                    "x": -315.16857910156244,
                    "y": 116.13861083984375
                },
                {
                    "id": "30",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "medium",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Kiali",
                            "offering": "oss",
                            "tags": [
                                "metric",
                                "monitoring",
                                "data",
                                "dashboard",
                                "servicemesh",
                                "observability"
                            ],
                            "category": "infrastructure"
                        }
                    }
                },
                {
                    "id": "31",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Thanos",
                            "offering": "oss",
                            "tags": [
                                "metric",
                                "monitoring",
                                "data",
                                "prometheus",
                                "ha"
                            ],
                            "category": "infrastructure"
                        }
                    },
                    "x": -596.3293762207031,
                    "y": 187.92852783203125
                },
                {
                    "id": "32",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Flux",
                            "offering": "oss",
                            "tags": [
                                "cd",
                                "gitops",
                                "kubernetes",
                                "fluxcd"
                            ],
                            "category": "infrastructure"
                        }
                    },
                    "x": -603.8572998046875,
                    "y": 84.79862976074219
                },
                {
                    "id": "33",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Circle CI",
                            "offering": "oss",
                            "tags": [
                                "cd",
                                "ci"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "hold"
                    },
                    "x": -369.4373474121093,
                    "y": 156.25180053710938
                },
                {
                    "id": "34",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "medium",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "GitLab",
                            "offering": "oss",
                            "tags": [
                                "git",
                                "cd",
                                "ci",
                                "source code control"
                            ],
                            "category": "infrastructure"
                        }
                    }
                },
                {
                    "id": "35",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Kustomize",
                            "offering": "oss",
                            "tags": [
                                "git",
                                "cd",
                                "ci"
                            ],
                            "category": "infrastructure"
                        }
                    },
                    "x": -604.9948120117188,
                    "y": -11.134710788726807
                },
                {
                    "id": "36",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Helm",
                            "offering": "oss",
                            "tags": [
                                "kubernetes",
                                "package manager",
                                "charts",
                                "cncf"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "trial"
                    },
                    "x": -225.70535278320307,
                    "y": 67.77313232421875
                },
                {
                    "id": "37",
                    "rating": {
                        "magnitude": "medium",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Jenkins",
                            "offering": "oss",
                            "tags": [
                                "ci",
                                "cd",
                                "build",
                                "automation"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "adopt"
                    },
                    "x": -83.700439453125,
                    "y": -6.616486549377441
                },
                {
                    "id": "38",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Spinnaker",
                            "offering": "oss",
                            "tags": [
                                "ci",
                                "cd",
                                "build",
                                "automation"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "assess"
                    },
                    "x": -222.46017456054682,
                    "y": 237.21780395507812
                },
                {
                    "id": "39",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Team City",
                            "offering": "oss",
                            "tags": [
                                "ci",
                                "cd",
                                "build",
                                "automation"
                            ],
                            "category": "infrastructure"
                        }
                    },
                    "x": -497.2535095214844,
                    "y": 265.59588623046875
                },
                {
                    "id": "40",
                    "rating": {
                        "magnitude": "large",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Azure DevOps",
                            "offering": "commercial",
                            "tags": [
                                "azure",
                                "arm",
                                "terraform",
                                "provision",
                                "ci",
                                "cd",
                                "build",
                                "automation"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "adopt"
                    },
                    "x": -133.468505859375,
                    "y": -46.74660110473633
                },
                {
                    "id": "41",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "GitHub Actions",
                            "offering": "oss",
                            "tags": [
                                "ci",
                                "cd",
                                "build",
                                "automation"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "assess"
                    },
                    "x": -340.5830993652343,
                    "y": -8.5234375
                },
                {
                    "id": "42",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Tekton CD",
                            "offering": "oss",
                            "tags": [
                                "ci",
                                "cd",
                                "build",
                                "automation"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "assess"
                    },
                    "x": -299.41247558593744,
                    "y": 41.263526916503906
                },
                {
                    "id": "43",
                    "rating": {
                        "magnitude": "tiny",
                        "experience": "long",
                        "timestamp": 1616404488431,
                        "scope": "Conclusion",
                        "comment": "no comment yet",
                        "author": "system generated",
                        "object": {
                            "label": "Travis CI",
                            "offering": "oss",
                            "tags": [
                                "ci",
                                "cd",
                                "build",
                                "automation"
                            ],
                            "category": "infrastructure"
                        },
                        "ambition": "assess"
                    },
                    "x": -323.94754028320307,
                    "y": -101.19953155517578
                }
            ]
        }
    ],
    "templates": [
        {
            "svg_id": "radarSVGContainer",
            "width": 1450,
            "height": 1100,
            "topLayer": "sectors",
            "selectedRing": 1,
            "selectedSector": 2,
            "rotation": 0,
            "maxRingRadius": 450,
            "sectorBoundariesExtended": true,
            "editMode": false,
            "defaultFont": {
                "color": "blue",
                "fontSize": "38px",
                "fontFamily": "Arial, Helvetica",
                "fontStyle": "italic",
                "fontWeight": "bold"
            },
            "title": {
                "text": "Conclusion Technology Radar",
                "x": -700,
                "y": -520,
                "font": {
                    "fontSize": "34px",
                    "fontFamily": "Courier"
                }
            },
            "colors": {
                "background": "#fFf",
                "grid": "#bbb",
                "inactive": "#ddd"
            },
            "ringConfiguration": {
                "outsideRingsAllowed": true,
                "font": {
                    "color": "purple",
                    "fontSize": "24px",
                    "fontFamily": "Arial, Helvetica",
                    "fontStyle": "normal",
                    "fontWeight": "normal"
                },
                "stroke": {
                    "strokeWidth": 4,
                    "strokeColor": "blue",
                    "strokeArray": "100 1"
                },
                "rings": [
                    {
                        "label": "Hold",
                        "width": 0.2
                    },
                    {
                        "label": "Assess",
                        "width": 0.2
                    },
                    {
                        "label": "Trial",
                        "width": 0.15
                    },
                    {
                        "label": "Adopt",
                        "width": 0.45
                    }
                ]
            },
            "sectorConfiguration": {
                "outsideSectorsAllowed": true,
                "font": {
                    "color": "#000",
                    "fontSize": "28px",
                    "fontFamily": "Arial, Helvetica",
                    "fontStyle": "normal",
                    "fontWeight": "normal"
                },
                "stroke": {
                    "strokeWidth": 2,
                    "strokeColor": "gray",
                    "strokeArray": "100 1"
                },
                "sectors": [
                    {
                        "label": "Data Management",
                        "angle": 0.2,
                        "backgroundImage": {}
                    },
                    {
                        "label": "Libraries & Frameworks",
                        "angle": 0.2,
                        "backgroundImage": {}
                    },
                    {
                        "label": "Infrastructure",
                        "angle": 0.25
                    },
                    {
                        "label": "Languages",
                        "angle": 0.2
                    },
                    {
                        "label": "Concepts & Methodology",
                        "angle": 0.15
                    }
                ]
            },
            "colorsConfiguration": {
                "label": "Maturity",
                "colors": [
                    {
                        "label": "Fresh",
                        "color": "green",
                        "enabled": true
                    },
                    {
                        "label": "Been Around",
                        "color": "blue",
                        "enabled": true
                    },
                    {
                        "label": "Very Mature",
                        "color": "gray",
                        "enabled": true
                    },
                    {
                        "label": "Intermediate",
                        "color": "pink",
                        "enabled": true
                    },
                    {
                        "label": "Unassigned",
                        "color": "white"
                    }
                ]
            },
            "sizesConfiguration": {
                "label": "Relevance",
                "sizes": [
                    {
                        "label": "Niche",
                        "size": 0.7,
                        "enabled": true
                    },
                    {
                        "label": "Medium    ",
                        "size": 0.9,
                        "enabled": false
                    },
                    {
                        "label": "Very relevant",
                        "size": 1.3,
                        "enabled": true
                    },
                    {
                        "label": "Regular",
                        "size": 4
                    },
                    {
                        "label": "Regular",
                        "size": 5,
                        "enabled": false
                    }
                ]
            },
            "shapesConfiguration": {
                "label": "Offering",
                "shapes": [
                    {
                        "label": "Commercial",
                        "shape": "square"
                    },
                    {
                        "label": "Open Source",
                        "shape": "diamond"
                    },
                    {
                        "label": "Label",
                        "shape": "rectangleHorizontal",
                        "enabled": false
                    },
                    {
                        "label": "Other",
                        "shape": "circle",
                        "enabled": false
                    },
                    {
                        "label": "Label",
                        "shape": "star",
                        "enabled": false
                    },
                    {
                        "label": "Label",
                        "shape": "rectangleVertical",
                        "enabled": false
                    },
                    {
                        "label": "Label",
                        "shape": "triangle",
                        "enabled": false
                    },
                    {
                        "label": "Label",
                        "shape": "ring",
                        "enabled": false
                    },
                    {
                        "label": "Label",
                        "shape": "plus",
                        "enabled": false
                    }
                ]
            }
        },
        {
            "svg_id": "radarSVGContainer",
            "width": 1450,
            "height": 1000,
            "topLayer": "rings",
            "selectedRing": 1,
            "selectedSector": 0,
            "rotation": 0,
            "maxRingRadius": 450,
            "sectorBoundariesExtended": false,
            "editMode": true,
            "defaultFont": {
                "color": "black",
                "fontSize": "38px",
                "fontFamily": "Arial, Helvetica",
                "fontStyle": "normal",
                "fontWeight": "normal"
            },
            "title": {
                "text": "AMIS Inzet Overzicht ",
                "x": -700,
                "y": -470,
                "font": {
                    "fontSize": "34px"
                }
            },
            "colors": {
                "background": "#FFF",
                "grid": "#bbb",
                "inactive": "#ddd"
            },
            "ringConfiguration": {
                "outsideRingsAllowed": true,
                "font": {
                    "color": "purple"
                },
                "rings": [
                    {
                        "label": "> 3 maanden",
                        "width": 0.25,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    },
                    {
                        "label": "> 1 maand",
                        "width": 0.25,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    },
                    {
                        "label": "< 1 maand",
                        "width": 0.25,
                        "opacity": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    },
                    {
                        "label": "Beschikbaar",
                        "width": 0.25,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    }
                ]
            },
            "sectorConfiguration": {
                "outsideSectorsAllowed": true,
                "font": {
                    "fontSize": "32px",
                    "fontFamily": "Arial, Helvetica"
                },
                "sectors": [
                    {
                        "label": "Frontend",
                        "angle": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    },
                    {
                        "label": "Integratie",
                        "angle": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    },
                    {
                        "label": "Platform Engineering",
                        "angle": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    },
                    {
                        "label": "Regie",
                        "angle": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    },
                    {
                        "label": "Platform Beheer",
                        "angle": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    }
                ]
            },
            "colorsConfiguration": {
                "colors": [
                    {
                        "label": "Super Status",
                        "color": "blue",
                        "enabled": true
                    },
                    {
                        "label": "Unassigned",
                        "color": "white",
                        "enabled": false
                    },
                    {
                        "label": "Unassigned",
                        "color": "white",
                        "enabled": false
                    },
                    {
                        "label": "Unassigned",
                        "color": "white",
                        "enabled": false
                    },
                    {
                        "label": "Unassigned",
                        "color": "white"
                    }
                ]
            },
            "sizesConfiguration": {
                "sizes": [
                    {
                        "label": "< 8 uur",
                        "size": 1,
                        "enabled": true
                    },
                    {
                        "label": "1-4 dagen",
                        "size": 2,
                        "enabled": true
                    },
                    {
                        "label": "Full Time",
                        "size": 3,
                        "enabled": true
                    }
                ]
            },
            "shapesConfiguration": {
                "shapes": [
                    {
                        "label": "enkele opties",
                        "shape": "square",
                        "enabled": true
                    },
                    {
                        "label": "goede opties",
                        "shape": "diamond",
                        "enabled": true
                    },
                    {
                        "label": "geen opties",
                        "shape": "rectangleHorizontal",
                        "enabled": true
                    },
                    {
                        "label": "Shape Label",
                        "shape": "circle",
                        "enabled": false
                    }
                ]
            }
        },
        {
            "svg_id": "radarSVGContainer",
            "width": 1450,
            "height": 1000,
            "topLayer": "sectors",
            "selectedRing": 1,
            "selectedSector": 0,
            "rotation": 0,
            "maxRingRadius": 450,
            "sectorBoundariesExtended": false,
            "editMode": true,
            "defaultFont": {
                "color": "black",
                "fontSize": "38px",
                "fontFamily": "Arial, Helvetica",
                "fontStyle": "normal",
                "fontWeight": "normal"
            },
            "title": {
                "text": "Project Management",
                "x": -700,
                "y": -470,
                "font": {
                    "fontSize": "34px"
                }
            },
            "colors": {
                "background": "#FFF",
                "grid": "#bbb",
                "inactive": "#ddd"
            },
            "ringConfiguration": {
                "outsideRingsAllowed": true,
                "font": {
                    "color": "purple"
                },
                "rings": [
                    {
                        "label": "Todo",
                        "width": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    },
                    {
                        "label": "Doing",
                        "width": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    },
                    {
                        "label": "Review",
                        "width": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    },
                    {
                        "label": "Done",
                        "width": 0.2,
                        "opacity": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    },
                    {
                        "label": "Live",
                        "width": 0.2,
                        "backgroundImage": {},
                        "backgroundColor": "white"
                    }
                ]
            },
            "sectorConfiguration": {
                "outsideSectorsAllowed": true,
                "font": {
                    "fontSize": "32px",
                    "fontFamily": "Arial, Helvetica"
                },
                "sectors": [
                    {
                        "label": "Epic 1",
                        "angle": 0.25,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    },
                    {
                        "label": "Epic 4",
                        "angle": 0.25,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    },
                    {
                        "label": "Epic 3",
                        "angle": 0.25,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    },
                    {
                        "label": "Epic 2",
                        "angle": 0.25,
                        "backgroundImage": {},
                        "backgroundColor": "white",
                        "outerringBackgroundColor": "#FFF"
                    }
                ]
            },
            "colorsConfiguration": {
                "colors": [
                    {
                        "label": "Recent",
                        "color": "#08c42a",
                        "enabled": true
                    },
                    {
                        "label": "Some Time",
                        "color": "#f2f25a",
                        "enabled": true
                    },
                    {
                        "label": "Quite Some Time",
                        "color": "#f29a1f",
                        "enabled": true
                    },
                    {
                        "label": "Very Long",
                        "color": "#f20039",
                        "enabled": true
                    },
                    {
                        "label": "Impediment",
                        "color": "#000000",
                        "enabled": true
                    }
                ]
            },
            "sizesConfiguration": {
                "sizes": [
                    {
                        "label": "Small",
                        "size": 1,
                        "enabled": true
                    },
                    {
                        "label": "Medium",
                        "size": 2,
                        "enabled": true
                    },
                    {
                        "label": "Large",
                        "size": 3,
                        "enabled": true
                    }
                ]
            },
            "shapesConfiguration": {
                "shapes": [
                    {
                        "label": "User Interface",
                        "shape": "square",
                        "enabled": true
                    },
                    {
                        "label": "API",
                        "shape": "diamond",
                        "enabled": true
                    },
                    {
                        "label": "Design",
                        "shape": "rectangleHorizontal",
                        "enabled": true
                    },
                    {
                        "label": "Test",
                        "shape": "circle",
                        "enabled": true
                    }
                ]
            }
        }
    ],
    "objects": []
}

sample.viewpoints[0].template = sample.templates[0]